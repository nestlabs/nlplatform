...
const nl_reset_reason_t kCrashTypeToResetReason[] =
{
    ...
    NL_RESET_REASON_STACK_OVERFLOW,   // DBGMON
};
...
typedef struct {
    ...
    uint32_t r11;
#if __CORTEX_M == 0x04
    // Cortex M4F has an extra word pushed that tells us if FP registers are in use
    // and also saved on the stack.  Right now we don't expect FP registers to
    // be saved so have this word, but don't look at it.  In the future, if we
    // start using HW FP, we need to check this value in order to find the pendSVFrame
    // location correctly on the stack.
    uint32_t r14_FP_flags;
#endif
    /* r0-r3, r12, lr, pc, psr are pushed by the PendSV HW interrupt of Cortex M3/M4 */
...
#ifdef FAULT_VERBOSE
static void putlabel(const char *label);
static void putreg(const char *label, uint32_t value);
#endif
...
void usage_fault(uint32_t *faultingStackAddress , uint32_t *machineStackAddress, crashType type) LINKER_REPLACEABLE_FUNCTION(usage_fault_default);
...
static uint32_t get_task_stack_top(const TaskHandle_t taskHandle)
{
    uint32_t stackTop;
...
{
     // must push registers in multiples of 2 to keep stack aligned on 8-bytes boundaries
    __asm volatile
    (
        " tst lr, #4                                                \n"
        " stmdb sp!, {r4-r12,lr}                                    \n"
...
void  __attribute__( ( naked ) ) nlfault_usage_fault_handler_c(void)
{
    __asm volatile
    (
        " tst lr, #4                                                \n"
...
static uint32_t *getRegisterPtr(uint32_t r, uint32_t *faultingStackAddress, uint32_t *machineStackAddress)
{
    if (r < 4)
        return &(faultingStackAddress[r]);
    else if (r < 12)
        return &(machineStackAddress[r-4]);
    else if (r == 12)
        return &(faultingStackAddress[4]);
    else if (r > 13)
        return &(faultingStackAddress[r-14+5]);
    else return 0;
}

void usage_fault_default( uint32_t *faultingStackAddress, uint32_t *machineStackAddress, crashType type);
void usage_fault_default( uint32_t *faultingStackAddress, uint32_t *machineStackAddress, crashType type)
{
    ExceptionStackFrame_t *faultFrame = (ExceptionStackFrame_t*)faultingStackAddress;
    uint32_t pc = faultFrame->pc;
    uint32_t armFaultInstruction = (*((uint16_t*) pc) << 16) | *(((uint16_t*) pc)+1);
    uint16_t thumbFaultInstruction = * ((uint16_t *) pc);
    //faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Usage fault: "));
    // usage fault due to alignment and one that we know how to fix
    if ((SCB->CFSR == SCB_CFSR_UNALIGNED) &&
        (((armFaultInstruction & 0xffc00000) == 0xe8800000) || // arm ldmia, stmia
         ((thumbFaultInstruction & 0xf000) == 0xc000))) // thumb ldmia, stmia
    {
        uint32_t reglist; // register list to ld/st
        uint32_t base; // register holding the pointer
        uint8_t writeback = TRUE;
        uint8_t ld = FALSE;
        uint32_t i;
        uint32_t *basePtr; // memory for actual loads/stores
        uint32_t *regPtr; // location where we can fetch the register value

        if ((thumbFaultInstruction & 0xf000) == 0xc000) {
            //faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING(" inst %x at %lx\n"), thumbFaultInstruction,pc);
            reglist = thumbFaultInstruction & 0x00ff;
            ld = thumbFaultInstruction & 0x0800 ? TRUE : FALSE;
            base = (thumbFaultInstruction >> 8) & 0x0007;
            pc += 2;
        } else {
            //faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING(" inst %lx at %lx\n"), armFaultInstruction,pc);
            reglist = armFaultInstruction & 0x0000ffff;
            writeback = armFaultInstruction & 0x00200000 ? TRUE: FALSE;
            ld = armFaultInstruction & 0x00100000? TRUE:FALSE;
            base = (armFaultInstruction >> 16) & 0x0f;
            pc += 4;
        }

        regPtr = getRegisterPtr(base, faultingStackAddress, machineStackAddress);
        if (regPtr == 0)
        {
            // this should really never happen: regPtr could be 0 for r13,
            // and if we're doing usage fault on SP pushes we've got
            // bigger problems and we will not attempt to handle that.
            goto crash;
        }
        basePtr = (uint32_t *) (*regPtr);

        for (i = 0; reglist; i++, reglist = reglist>> 1) {
            if (reglist & 0x01) {
                regPtr = getRegisterPtr(i, faultingStackAddress, machineStackAddress);
                if (regPtr == 0)
                {
                    goto crash;
                }
        ...
        if (writeback)
        {
            ...
            *regPtr = (uint32_t) basePtr;
        }
        // fixup the return address
        faultFrame->pc = pc;
...
crash:
    crash_dump(faultingStackAddress, machineStackAddress, type);
}
...
void  __attribute__( ( naked ) ) nlfault_debug_monitor_handler_c(void)
{
     __asm volatile
    (
        " mov r2, #2                                                \n"
...
static void putlabel(const char *label)
{
    if (label != NULL)
    {
        while (*label)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("%c"), *label++);
        }
    }
}
...
static uint32_t threadDump(TaskStatus_t *task_status, char *task_name, char *task_state, uint32_t *backtrace_buf, ExceptionStackFrame_t *custom_exception_frame)
{
    ...
    strncpy(task_name, pcTaskGetName(task_status->xHandle), TASK_NAME_LEN-1);
...
void dump_all_tasks_default(bool task_fault)
{
    ...
    unsigned backtrace_idx = 0;
    TaskHandle_t active_task_handle = xTaskGetCurrentTaskHandle();
    ...
    if (save_fault_to_reset_info)
    {
        ...
        else
        {
            ...
            strncpy(g_reset_info.fault_info.active_task_name, pcTaskGetName(active_task_handle), NL_FAULT_DIAGS_TASK_NAME_LEN - 1);
            strncpy(g_reset_info.fault_info.task_info[0].task_name, pcTaskGetName(active_task_handle), NL_FAULT_DIAGS_TASK_NAME_LEN - 1);
            g_reset_info.fault_info.task_info[0].task_state[0] = tskACTIVE_CHAR;
            backtrace_idx = 1;
        }
    }
...
static void faultDecode(uint32_t lr, uint32_t pc)
{
#ifdef FAULT_VERBOSE
    uint32_t MMFAR = 0;
    uint32_t BFAR = 0;
    ...
    {
        faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Vector Table Hard Fault\n"));
    }
    else if ((SCB->HFSR & SCB_HFSR_FORCED_Msk) != 0)
    {
        faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Other Hard Fault\n"));

        if ((SCB->CFSR & SCB_CFSR_MMARVALID    ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Memory Management Fault\n"));
            MMFAR = SCB->MMFAR;
            putreg("mmfar = ", MMFAR);
        }
        ...
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Bus Fault\n"));
            BFAR = SCB->BFAR;
            putreg("bfar = ", BFAR);
        }
        ...
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Instruction Access Violation Fault\n"));
            putreg("lr = ", lr);
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_DACCVIOL     ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Data Access Violation Fault\n"));
            putreg("pc = ", pc);
            putreg("mmfar = ", MMFAR);
        }
        else if ((SCB->CFSR & SCB_CFSR_MUNSTKERR    ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Memory Unstacking Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_MSTKERR      ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Memory Stacking Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_IBUSERR      ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Instruction Bus Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_PRECISERR    ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Precise Data Bus Error Fault\n"));
            putreg("pc = ", pc);
            putreg("bfar = ", BFAR);
        }
        else if ((SCB->CFSR & SCB_CFSR_IMPRECISERR  ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Imprecise Data Bus Error Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_UNSTKERR     ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Bus Unstacking Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_STKERR       ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Bus Stacking Fault\n"));
        }
        else if ((SCB->CFSR & SCB_CFSR_UNDEFINSTR   ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Undefined Instruction Usage Fault\n"));
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_INVSTATE     ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Invalid State Fault\n"));
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_INVPC        ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Invalid Program Counter Fault\n"));
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_NOCP         ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("No Coprocessor Fault\n"));
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_UNALIGNED   ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Unaligned Access Fault\n"));
            putreg("pc = ", pc);
        }
        else if ((SCB->CFSR & SCB_CFSR_DIVBYZERO    ) != 0)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Divide By Zero Fault\n"));
            putreg("pc = ", pc);
        }
        else
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Unknown Fault\n"));
        }
    }
    else if (SCB->DFSR)
    {
        ...
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("External debug event\n"));
        }
        if (SCB->DFSR & SCB_DFSR_VCATCH_Msk)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Vector catch\n"));
        }
        if (SCB->DFSR & SCB_DFSR_DWTTRAP_Msk)
        {
            ...
            comp_regs_t *comp_regs = (comp_regs_t *)&DWT->COMP0;
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("DWT debug event\n"));
            for (i = 0; i < (DWT->CTRL >> DWT_CTRL_NUMCOMP_Pos); i++, comp_regs++)
            {
                // dump comparator register if enabled
                if (comp_regs->FUNCTION & DWT_FUNCTION_FUNCTION_Msk)
                {
                    faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("DWT->COMP[%u] = 0x%08lx\n"), i, comp_regs->COMP);
                    faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("DWT->MASK[%u] = 0x%08lx\n"), i, comp_regs->MASK);
                    faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("DWT->FUNCTION[%u] = 0x%08lx\n"), i, comp_regs->FUNCTION);
                }
             }
        }
        if (SCB->DFSR & SCB_DFSR_BKPT_Msk)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Breakpoint debug event\n"));
        }
        if (SCB->DFSR & SCB_DFSR_HALTED_Msk)
        {
            faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("HALT debug event\n"));
        }
    }
    else
    {
        faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Unknown Hard Fault\n"));
    }
#else // !FAULT_VERBOSE
    faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("Fault! %08lx %08lx %08lx %08lx %08lx\n"),
                    SCB->HFSR, SCB->CFSR, SCB->MMFAR, SCB->BFAR, SCB->DFSR);
#endif // FAULT_VERBOSE
}
...
void dump_stack_overflow_info_to_reset_info_default(const char *current_task_name)
{
    size_t i = append_string_to_fault_info_description(0, UNIQUE_STRING_LITERAL("stack_overflow"));
...
uint32_t *dump_context_to_reset_info_default(uint32_t *faultingStackAddress , uint32_t *machineStackAddress, nl_reset_reason_t reset_reason, uint32_t prefault_sp, const char *current_task_name, bool task_fault)
{
        ...
        (g_reset_info.fault_info.reason == NL_RESET_REASON_ASSERT))
    {
        ...
        // clear all entries in task_info
        memset(g_reset_info.fault_info.task_info, 0, sizeof(g_reset_info.fault_info.task_info));
        // clear machine_backtrace buffer
 ...
 * need this). Can't use UNIQUE_PRINTF_FORMAT_STRING() in
 * this case because we're using the same literal string twice.
 */
#ifdef BUILD_FEATURE_PRINTF_TOKENIZATION
#define PRINT_REG_FORMAT_STRING "r%d\t0x%08lx\n"
#else
#ifdef DEBUG
static const char PRINT_REG_FORMAT_STRING[] = "r%d\t0x%08lx\n";
#endif // DEBUG
#endif // BUILD_FEATURE_PRINTF_TOKENIZATION
...
void dump_context_default(uint32_t *faultingStackAddress , uint32_t *machineStackAddress, crashType type, uint32_t prefault_sp)
{
    ...
    {
        faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("DbgMon - StackOverflow\n"));
    }

    // fault type dump
    faultDecode(faultFrame->lr, faultFrame->pc);
    ...
    {
        faultDebugPrint(PRINT_REG_FORMAT_STRING, i, faultingStackAddress[i]);
    }

    // register dump: r4-r11
    for (i = 0; i < NUM_REGISTERS_ON_MAIN_STACK; i++)
    {
        faultDebugPrint(PRINT_REG_FORMAT_STRING, i+4, machineStackAddress[i]);
    }
                            ...
                            uint32_t prefault_sp, uint32_t stackTop, nl_reset_reason_t reset_reason)
{
    ...
    // If the nlbacktrace_with_lr() returned no backtrace (possibly due to MPU preventing
    // an exception frame from being pushed onto the faulting stack), or if the fault
    // was due to a stack overflow but returned too few levels (again, bad pc or lr),
    // then use nlbacktrace_no_context() to get a longer backtrace.
    if ((size == 0) || ((reset_reason == NL_RESET_REASON_STACK_OVERFLOW) && (size < MIN_BACKTRACE_LEVELS_FOR_STACK_OVERFLOW)))
    {
...
void get_and_dump_current_task_info_default(const char **name, nl_reset_reason_t *reset_reason, uint32_t *stackTop)
{
    ...
    current_task_name = (const char*)pcTaskGetName(handle);
    ...
    *name = current_task_name;
    ...
    // If the MPU is used to guard stacks, it could prevent an exception
    // frame from being pushed onto the task stack.  This can be detected
    // by the MSTKERR bit in the CFSR register.  Print a message if this
    // bit is set to warn that the register dump could be garbage.
    if (SCB->CFSR & SCB_CFSR_MSTKERR)
    {
        faultDebugPrint(UNIQUE_PRINTF_FORMAT_STRING("MemManage fault on saving exception context, likely stack overflow\n"));
        *reset_reason = NL_RESET_REASON_STACK_OVERFLOW;
    }
}
...
#if !defined(BUILD_CONFIG_RELEASE) || defined(BUILD_FEATURE_RESET_INFO) || defined(BUILD_FEATURE_BREADCRUMBS)
 ...
 */
void nlfault_freertos_stack_overflow_handler_c(TaskHandle_t pxTask, signed char *pcTaskName);
void nlfault_freertos_stack_overflow_handler_c(TaskHandle_t pxTask, signed char *pcTaskName)
{
